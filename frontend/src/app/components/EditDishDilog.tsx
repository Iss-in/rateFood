...
interface EditDishDialogProps {
  ...
  onEditDish: (dish: Omit<Dish, "id" | "favoriteCount">) => void;
}
...
// Reuse the same RestaurantNameDropdown component from AddDishDialog
...
export function EditDishDialog({ dish, selectedCity, open, onOpenChange, onEditDish }: EditDishDialogProps) {
  const [isRestaurantValid, setIsRestaurantValid] = useState(true); // Start as valid since we're editing
  ...
  // Reset form data when dish changes or dialog opens
  useEffect(() => {
    if (open && dish) {
      setFormData({
        name: dish.name,
